...
foreach($_SESSION['cart'] as $key => $value)
{
    ...
    while (mysqli_stmt_fetch($query)){
        echo '<div class="product-card"><div class="image" style="background: url(\'' . $product_image . '\') center no-repeat; background-size: cover;"></div><div class="description"><p class="categories">Chemise, Homme</p><p class="name">' . $product_name . '</p><p class="price">$' . $product_price . '</p><div class="cart-act-wrapper"><a class="cart-act" href="handlers/addtocart.php?product_id=' . $product_id .'">+ 1</a>|<a class="cart-act" href="handlers/subfromcart.php?product_id=' . $product_id .'">- 1</a>|<a class="cart-act" href="handlers/delfromcart.php?product_id=' . $product_id .'">Remove</a></div></div></div>';
    }
}
